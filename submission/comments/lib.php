 ...
 * @return bool
 */
function fastassignsubmission_comments_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if (!$submission = $DB->get_record('fastassignment_submission', array('id'=>$options->itemid))) {
 ...
 * @return array
 */
function fastassignsubmission_comments_comment_permissions(stdClass $options) {
    global $USER, $CFG, $DB;
 ...
 */
function fastassignsubmission_comments_comment_display($comments, $options) {
    global $CFG, $DB, $USER;
    ...
    if ($assignment->is_blind_marking() && !empty($comments)) {
        ...
        $userinteam = false;
        if ($assignment->get_instance()->teamsubmission && has_capability('mod/fastassignment:submit', $context)) {
        ...
        foreach ($comments as $comment) {
            ...
            if (has_capability('mod/fastassignment:viewblinddetails', $context) && $USER->id != $comment->userid) {
                ...
                $comment->fullname = get_string('blindmarkingviewfullname', 'fastassignsubmission_comments', $a);
                ...
                if (empty($usermappings[$comment->userid])) {
                    ...
                    $commenter->firstname = get_string('blindmarkingname', 'fastassignsubmission_comments', $anonid);
                    $commenter->lastname = '';
 ...
 */
function fastassignsubmission_comments_comment_add(stdClass $comment, stdClass $param) {
