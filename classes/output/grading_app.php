...
namespace mod_fastassignment\output;
...
class grading_app implements templatable, renderable {
    ...
    public function __construct($userid, $groupid, $assignment) {
        ...
        user_preference_allow_ajax_update('fastassignment_filter', PARAM_ALPHA);
        user_preference_allow_ajax_update('fastassignment_workflowfilter', PARAM_ALPHA);
        user_preference_allow_ajax_update('fastassignment_markerfilter', PARAM_ALPHANUMEXT);
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $export->viewgrading = get_string('viewgrading', 'mod_fastassignment');
        ...
        $export->caneditsettings = has_capability('mod/fastassignment:addinstance', $this->assignment->get_context());
        ...
        if ($export->duedate - $time <= 0) {
            $due = get_string('assignmentisdue', 'fastassignment');
        } else {
            $due = get_string('timeremainingcolon', 'fastassignment', format_time($export->duedate - $time));
        }
        ...
        if ($export->duedate < $time) {
            ...
            if ($cutoffdate) {
                if ($cutoffdate > $time) {
                    $late = get_string('latesubmissionsaccepted', 'fastassignment', userdate($export->cutoffdate));
                } else {
                    $late = get_string('nomoresubmissionsaccepted', 'fastassignment');
                }
        ...
        $helpicon = new \help_icon('sendstudentnotifications', 'fastassignment');
