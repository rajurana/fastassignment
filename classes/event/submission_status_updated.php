...
/**
 * The mod_fastassignment submission status updated event.
 *
...
/**
 * The mod_fastassignment submission status updated event class.
 *
...
class submission_status_updated extends base {
     ...
     */
    public static function create_from_submission(\fastassignment $fastassignment, \stdClass $submission) {
        $data = array(
            'context' => $fastassignment->get_context(),
            'objectid' => $submission->id,
            'relateduserid' => ($fastassignment->get_instance()->teamsubmission) ? null : $submission->userid,
            'other' => array(
        ...
        $event->add_record_snapshot('fastassignment_submission', $submission);
        return $event;
    }
    ...
    public static function get_name() {
        return get_string('eventsubmissionstatusupdated', 'mod_fastassignment');
    }
    ...
    protected function get_legacy_logdata() {
        $submission = $this->get_record_snapshot('fastassignment_submission', $this->objectid);
        $user = $this->get_record_snapshot('user', $submission->userid);
        $logmessage = get_string('reverttodraftforstudent', 'fastassignment', array('id' => $user->id, 'fullname' => fullname($user)));
