...
/**
 * Settings form for overrides in the fastassignment module.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the fastassignment settings object. */
    protected $fastassignment;

    /** @var context the fastassignment context. */
    protected $context;
     ...
     * @param object $fastassignment the fastassignment settings object.
     * @param object $context the fastassignment context.
     ...
     */
    public function __construct($submiturl, $cm, $fastassignment, $context, $groupmode, $override, $selecteduserid = null) {
    ...
    protected function definition() {
        ...
        $assigninstance = $this->fastassignment->get_instance($userid);
        $inrelativedatesmode = !empty($this->fastassignment->get_course()->relativedatesmode);

        $mform->addElement('header', 'override', get_string('override', 'fastassignment'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'fastassignment'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/fastassignment/overrides.php', array('cmid' => $cm->id));
                    print_error('groupsnone', 'fastassignment', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'fastassignment'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/fastassignment/overrides.php', array('cmid' => $cm->id));
                    print_error('usersnone', 'fastassignment', $link);
                }
                ...
                if ($inrelativedatesmode) {
                    ...
                    $PAGE->requires->js_call_amd('mod_fastassignment/override_form', 'init', [
            ...
            if ($inrelativedatesmode) {
                if ($userid) {
                    ...
                    $html = $OUTPUT->render_from_template('mod_fastassignment/override_form_user_defaults', $templatecontext);
                } else {
                ...
                $mform->addGroup($groupelements, null, get_string('userassignmentdefaults', 'mod_fastassignment'), null, false);
            }
        }
        ...
        foreach ($users as $value) {
            $extension = $DB->get_record('fastassignment_user_flags', array('assignment' => $assigninstance->id,
                'userid' => $value));
            if ($extension) {
        ...
        $mform->addElement('date_time_selector', 'allowsubmissionsfromdate',
            get_string('allowsubmissionsfromdate', 'fastassignment'), array('optional' => true));
        $mform->setDefault('allowsubmissionsfromdate', $assigninstance->allowsubmissionsfromdate);

        $mform->addElement('date_time_selector', 'duedate', get_string('duedate', 'fastassignment'), array('optional' => true));
        $mform->setDefault('duedate', $assigninstance->duedate);

        $mform->addElement('date_time_selector', 'cutoffdate', get_string('cutoffdate', 'fastassignment'), array('optional' => true));
        $mform->setDefault('cutoffdate', $assigninstance->cutoffdate);

        if (isset($assigninstance->extensionduedate)) {
            $mform->addElement('static', 'extensionduedate', get_string('extensionduedate', 'fastassignment'),
                userdate($assigninstance->extensionduedate));
        }

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'fastassignment'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'fastassignment'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'fastassignment'));
    ...
    public function validation($data, $files) {
        ...
        if (!empty($assigninstance->extensionduedate) && !empty($data['duedate'])) {
            if ($assigninstance->extensionduedate < $data['duedate']) {
                $errors['duedate'] = get_string('extensionnotafterduedate', 'fastassignment');
            }
        }
        if (!empty($assigninstance->extensionduedate) && !empty($data['allowsubmissionsfromdate'])) {
            if ($assigninstance->extensionduedate < $data['allowsubmissionsfromdate']) {
                $errors['allowsubmissionsfromdate'] = get_string('extensionnotafterfromdate', 'fastassignment');
            }
        }

        // Ensure that at least one fastassignment setting was changed.
        $changed = false;
        ...
        if (!$changed) {
            $errors['allowsubmissionsfromdate'] = get_string('nooverridedata', 'fastassignment');
        }
