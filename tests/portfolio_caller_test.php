...
/**
 * Tests behaviour of the fastassignment_portfolio_caller class.
 *
...
/**
 * Class mod_fastassignment_portfolio_caller_testcase
 *
 * Tests behaviour of the fastassignment_portfolio_caller class.
 *
 * @copyright Brendan Cox <brendan.cox@totaralearning.com>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_fastassignment_portfolio_caller_testcase extends advanced_testcase {
    ...
    public function test_user_submission_file_is_loaded() {
        ...
        /* @var mod_fastassignment_generator $assigngenerator */
        $assigngenerator = $this->getDataGenerator()->get_plugin_generator('mod_fastassignment');

        $activityrecord = $assigngenerator->create_instance(array('course' => $course->id));
        $cm = get_coursemodule_from_instance('fastassignment', $activityrecord->id);
        ...
        $caller = new fastassignment_portfolio_caller(array('cmid' => $cm->id, 'fileid' => $file->get_id()));
        ...
        $caller = new fastassignment_portfolio_caller(
            array(
                'cmid' => $cm->id,
                'sid' => $submission->id,
                'area' => FASTASSIGNSUBMISSION_FILE_FILEAREA,
                'component' => 'fastassignsubmission_file',
            )
        );
        $caller->set('user', $user);
    ...
    public function test_group_submission_file_is_loaded() {
        ...
        $submission = $fastassignment->get_group_submission($user->id, $groupid, true);
