 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_submit_for_grading_page implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_gradingmessage implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_form implements renderable {
    /** @var moodleform $form is the edit submission form */
    public $form = null;
    /** @var string $classname is the name of the class to fastassignment to the container */
    public $classname = '';
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_user_summary implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_feedback_plugin_feedback implements renderable {
    /** @var int SUMMARY */
    const SUMMARY                = 10;
    /** @var int FULL */
    const FULL                   = 20;

    /** @var fastassignment_submission_plugin $plugin */
    public $plugin = null;
    /** @var stdClass $grade */
    public $grade = null;
    /** @var string $view */
     ...
     *
     * @param fastassignment_feedback_plugin $plugin
     ...
     */
    public function __construct(fastassignment_feedback_plugin $plugin,
                                stdClass $grade,
                                $view,
                                $coursemoduleid,
                                $returnaction,
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_submission_plugin_submission implements renderable {
    /** @var int SUMMARY */
    const SUMMARY                = 10;
    /** @var int FULL */
    const FULL                   = 20;
    ...
    /**
     * Constructor
     * @param fastassignment_submission_plugin $plugin
     ...
     */
    public function __construct(fastassignment_submission_plugin $plugin,
                                stdClass $submission,
                                $view,
                                $coursemoduleid,
                                $returnaction,
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_feedback_status implements renderable {
    ...
    /** @var stdClass fastassignment_grade record */
    public $grade = null;
    /** @var int coursemoduleid */
    public $coursemoduleid = 0;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_submission_status implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_submission_status_compact extends fastassignment_submission_status implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_attempt_history implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_attempt_history_chooser implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        foreach ($export->submissions as $i => $submission) {
            ...
            $submission->attemptsummary = get_string('attemptheading', 'fastassignment', $attemptsummaryparams);
            $submission->statussummary = get_string('submissionstatus_' . $submission->status, 'fastassignment');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_header implements renderable {
    /** @var stdClass the fastassignment record  */
    public $fastassignment = null;
     ...
     * Constructor
     *
     * @param stdClass $fastassignment  - the fastassignment database record
     ...
     */
    public function __construct(stdClass $fastassignment,
                                $context,
                                $showintro,
                                $coursemoduleid,
                                $subpage='',
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_plugin_header implements renderable {
    /** @var fastassignment_plugin $plugin */
    public $plugin = null;

    /**
     * Header for a single plugin
     *
     * @param fastassignment_plugin $plugin
     */
    public function __construct(fastassignment_plugin $plugin) {
        $this->plugin = $plugin;
    }
}
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_grading_summary implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_course_index_summary implements renderable {
     ...
     */
    public function add_fastassignment_info($cmid, $cmname, $sectionname, $timedue, $submissioninfo, $gradeinfo) {
...
/**
 * An fastassignment file class that extends rendererable class and is used by the fastassignment module.
 *
 * @package   mod_fastassignment
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_files implements renderable {
    /** @var context $context */
    public $context;
    /** @var string $context */
    public $dir;
    ...
    public function __construct(context $context, $sid, $filearea, $component) {
        ...
        if (!empty($CFG->enableportfolios)) {
            ...
            if (count($files) >= 1 && !empty($sid) &&
                    has_capability('mod/fastassignment:exportownsubmission', $this->context)) {
                $button = new portfolio_add_button();
                $callbackparams = array('cmid' => $this->cm->id,
                                        'sid' => $sid,
                                        'area' => $filearea,
                                        'component' => $component);
                $button->set_callback_options('fastassignment_portfolio_caller',
                                              $callbackparams,
                                              'mod_fastassignment');
    ...
    public function preprocess($dir, $filearea, $component) {
        ...
        foreach ($dir['files'] as $file) {
            ...
            if (!empty($CFG->enableportfolios)) {
                require_once($CFG->libdir . '/portfoliolib.php');
                $button = new portfolio_add_button();
                if (has_capability('mod/fastassignment:exportownsubmission', $this->context)) {
