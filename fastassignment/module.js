M.mod_fastassignment = {};

M.mod_fastassignment.init_tree = function(Y, expand_all, htmlid) {
...
M.mod_fastassignment.init_grading_table = function(Y) {
        ...
        if (batchform) {
            batchform.on('submit', function(e) {
                ...
                if (selectedusers.length == 0) {
                    alert(M.util.get_string('nousersselected', 'fastassignment'));
                    e.preventDefault();
                } else {
                    ...
                    if (action.indexOf(prefix) == 0) {
                        ...
                        confirmmessage = M.util.get_string('batchoperationconfirm' + action, 'fastassignfeedback_' + plugin);
                    } else {
                        confirmmessage = M.util.get_string('batchoperationconfirm' + operation.get('value'), 'fastassignment');
                    }
...
M.mod_fastassignment.init_grading_options = function(Y) {
    Y.use('node', function(Y) {
...
M.mod_fastassignment.init_plugin_summary = function(Y, subtype, type, submissionid) {
