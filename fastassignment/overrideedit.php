...
/**
 * This page handles editing and creation of fastassignment overrides
 *
...
$pagetitle = get_string('editoverride', 'fastassignment');

$override = null;
if ($overrideid) {

    if (! $override = $DB->get_record('fastassignment_overrides', array('id' => $overrideid))) {
        print_error('invalidoverrideid', 'fastassignment');
    }

    list($course, $cm) = get_course_and_cm_from_instance($override->assignid, 'fastassignment');
...
$url = new moodle_url('/mod/fastassignment/overrideedit.php');
if ($action) {
...
$assigninstance = $fastassignment->get_instance($userid);
...
// Merge fastassignment defaults with data.
...
if ($action === 'duplicate') {
    ...
    $pagetitle = get_string('duplicateoverride', 'fastassignment');
}
...
$overridelisturl = new moodle_url('/mod/fastassignment/overrides.php', array('cmid' => $cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new fastassignment_override_form($url, $cm, $fastassignment, $context, $groupmode, $override, $userid);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('fastassignment_overrides', $conditions)) {
            ...
            $fastassignment->delete_override($oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('fastassignment_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('fastassignment_overrides', $fromform);
        if ($groupmode) {
            $fromform->sortorder = 1;

            $overridecountgroup = $DB->count_records('fastassignment_overrides',
                array('userid' => null, 'assignid' => $assigninstance->id));
            $overridecountall = $DB->count_records('fastassignment_overrides', array('assignid' => $assigninstance->id));
            ...
            $DB->update_record('fastassignment_overrides', $fromform);
            reorder_group_overrides_fast($assigninstance->id);
        }
    ...
    fastassignment_update_events($fastassignment, $fromform);
