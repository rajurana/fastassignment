...
/**
 * Define all the restore steps that will be used by the restore_fastassignment_activity_task
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_fastassignment_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('fastassignment', '/activity/fastassignment');
        if ($userinfo) {
            $submission = new restore_path_element('fastassignment_submission',
                                                   '/activity/fastassignment/submissions/submission');
            $paths[] = $submission;
            $this->add_subplugin_structure('fastassignsubmission', $submission);
            $grade = new restore_path_element('fastassignment_grade', '/activity/fastassignment/grades/grade');
            $paths[] = $grade;
            $this->add_subplugin_structure('fastassignfeedback', $grade);
            $userflag = new restore_path_element('fastassignment_userflag',
                                                   '/activity/fastassignment/userflags/userflag');
            $paths[] = $userflag;
        }

        $paths[] = new restore_path_element('fastassignment_override', '/activity/fastassignment/overrides/override');
        $paths[] = new restore_path_element('fastassignment_plugin_config',
                                            '/activity/fastassignment/plugin_configs/plugin_config');

        return $this->prepare_activity_structure($paths);
    }

    /**
     * Process an fastassignment restore.
     *
    ...
    protected function process_assign($data) {
        ...
        $newitemid = $DB->insert_record('fastassignment', $data);
     ...
     * @return void
     */
    protected function process_fastassignment_submission($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('fastassignment_submission', $data);
     ...
     * @return void
     */
    protected function process_fastassignment_userflag($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('fastassignment_user_flags', $data);
    }
     ...
     * @return void
     */
    protected function process_fastassignment_grade($data) {
        global $DB;
        ...
        if (!empty($data->extensionduedate) ||
                ...
                !empty($data->locked)) {
            $flags = new stdClass();
            $flags->assignment = $this->get_new_parentid('fastassignment');
            if (!empty($data->extensionduedate)) {
        ...
        if ($data->grade < 0 && $data->grade != FASTASSIGN_GRADE_NOT_SET) {
            $data->grade = FASTASSIGN_GRADE_NOT_SET;
        }
        $newitemid = $DB->insert_record('fastassignment_grades', $data);
     ...
     * @return void
     */
    protected function process_fastassignment_plugin_config($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('fastassignment_plugin_config', $data);
    }
    ...
    protected function set_latest_submission_field() {
        ...
        $assignmentid = $this->get_new_parentid('fastassignment');
        ...
        $DB->insert_records('fastassignment_submission', $submissions);
        ...
        $sql = 'SELECT DISTINCT userid FROM {fastassignment_submission} WHERE assignment = ? AND groupid = ?';
        ...
        $sql = 'SELECT DISTINCT groupid FROM {fastassignment_submission} WHERE assignment = ? AND userid = ?';
        $params = array($assignmentid, 0);
    ...
    protected function add_plugin_config_files($subtype) {
        $dummyassign = new fastassignment(null, null, null);
    ...
    /**
     * Process a fastassignment override restore
     * @param object $data The data in object form
     * @return void
     */
    protected function process_fastassignment_override($data) {
        global $DB;
        ...
        $data->assignid = $this->get_new_parentid('fastassignment');
        ...
        $newitemid = $DB->insert_record('fastassignment_overrides', $data);

        // Add mapping, restore of logs needs it.
        $this->set_mapping('fastassignment_override', $oldid, $newitemid);
    }
    ...
    protected function after_execute() {
        $this->add_related_files('mod_fastassignment', 'intro', null);
        $this->add_related_files('mod_fastassignment', 'introattachment', null);

        $this->add_plugin_config_files('fastassignsubmission');
        $this->add_plugin_config_files('fastassignfeedback');
