...
/**
 * The mod_fastassignment remove submission form viewed event.
 *
...
/**
 * The mod_fastassignment remove submission form viewed event class.
 *
 * @package    mod_fastassignment
 * @copyright  2019 Damyon Wiese
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class remove_submission_form_viewed extends base {
     ...
     */
    public static function create_from_user(\fastassignment $fastassignment, \stdClass $user) {
        $data = array(
    ...
    public static function get_name() {
        return get_string('eventremovesubmissionformviewed', 'mod_fastassignment');
    }
    ...
    protected function get_legacy_logdata() {
        if ($this->relateduserid == $this->userid) {
            $title = get_string('removesubmission', 'fastassignment');
        } else {
            $user = $this->get_record_snapshot('user', $this->relateduserid);
            $title = get_string('removesubmissionforstudent', 'fastassignment', array('id' => $user->id, 'fullname' => fullname($user)));
        }
