...
/**
 * This file contains the upgrade code to upgrade from mod_fastassignmentment to mod_fastassignment
 *
 ...
 */
define('FASTASSIGN_MAX_UPGRADE_TIME_SECS', 300);

/**
 * Class to manage upgrades from mod_fastassignmentment to mod_fastassignment
 *
 * @package   mod_fastassignment
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_upgrade_manager {
    ...
    public function upgrade_assignment($oldassignmentid, & $log) {
        ...
        core_php_time_limit::raise(FASTASSIGN_MAX_UPGRADE_TIME_SECS);
        ...
        // during restore of a mod_fastassignmentment module.
        // Also note that we do not check for any mod_fastassignmentment capabilities, because they can
        // be removed so that users don't add new instances of the broken old thing.
        if (!has_capability('mod/fastassignment:addinstance', $oldcontext)) {
            $log = get_string('couldnotcreatenewassignmentinstance', 'mod_fastassignment');
            return false;
        }

        // First insert an fastassignment instance to get the id.
        ...
        $data->maxattempts = FASTASSIGN_UNLIMITED_ATTEMPTS;

        $newassignment = new fastassignment(null, null, null);
        ...
        $newmodule = $DB->get_record('modules', array('name'=>'fastassignment'), '*', MUST_EXIST);
        $newcoursemodule = $this->duplicate_course_module($oldcoursemodule,
                                                          $newmodule->id,
                                                          $newassignment->get_instance()->id);
        if (!$newcoursemodule) {
            $log = get_string('couldnotcreatenewcoursemodule', 'mod_fastassignment');
            return false;
        }
        ...
        try {
            ...
            // Copy intro files.
            $newassignment->copy_area_files_for_upgrade($oldcontext->id, 'mod_fastassignmentment', 'intro', 0,
                                            $newassignment->get_context()->id, 'mod_fastassignment', 'intro', 0);
            ...
            foreach ($allcriteria as $criteria) {
                $criteria->module = 'fastassignment';
            ...
            $DB->set_field('log', 'module', 'fastassignment', $logparams);
            ...
            foreach ($oldsubmissions as $oldsubmission) {
                ...
                // Because in mod_fastassignmentment there could only be one submission per student, it is always the latest.
                $submission->latest = 1;
                $submission->id = $DB->insert_record('fastassignment_submission', $submission);
                if (!$submission->id) {
                    $log .= get_string('couldnotinsertsubmission', 'mod_fastassignment', $submission->userid);
                    $rollback = true;
                }
                foreach ($newassignment->get_submission_plugins() as $plugin) {
                    if ($plugin->can_upgrade($oldassignment->assignmenttype, $oldversion)) {
                ...
                if ($oldsubmission->timemarked) {
                    ...
                    $grade->id = $DB->insert_record('fastassignment_grades', $grade);
                    if (!$grade->id) {
                        $log .= get_string('couldnotinsertgrade', 'mod_fastassignment', $grade->userid);
                        $rollback = true;
                    }
            ...
            // Reassociate grade_items from the old assignment instance to the new fastassignment instance.
            // This includes outcome linked grade_items.
            $params = array('fastassignment', $newassignment->get_instance()->id, 'assignment', $oldassignment->id);
            ...
            $rollback = true;
            $log .= get_string('conversionexception', 'mod_fastassignment', $exception->getMessage());
        }

        if ($rollback) {
            // Roll back the grades changes.
            if ($gradesdone) {
                // Reassociate grade_items from the new fastassignment instance to the old assignment instance.
                $params = array('assignment', $oldassignment->id, 'fastassignment', $newassignment->get_instance()->id);
                $sql = 'UPDATE {grade_items} SET itemmodule = ?, iteminstance = ? WHERE itemmodule = ? AND iteminstance = ?';
                $DB->execute($sql, $params);
            }
            ...
            if ($gradingarea) {
                ...
                $params = array('id'=>$gradingarea->id,
                                ...
                                'component'=>'mod_fastassignmentment',
                                'areaname'=>'submission');
                $DB->update_record('grading_areas', $params);
            }
     ...
     * fastassignment module alongside the old assignment module.
     *
     * @param stdClass $cm The old course module record
     * @param int $moduleid The id of the new fastassignment module
     * @param int $newinstanceid The id of the new instance of the fastassignment module
...
function get_assignments_with_rescaled_null_grades() {
    ...
    $query = 'SELECT id, assignment FROM {fastassignment_grades}
