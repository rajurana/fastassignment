...
/**
 * Unit tests for mod/fastassignment/feedback/comments/classes/privacy/
 *
 * @copyright  2018 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignfeedback_comments_privacy_testcase extends \mod_fastassignment\tests\mod_fastassignment_privacy_testcase {
    ...
    protected function create_feedback($fastassignment, $student, $teacher, $submissiontext, $feedbacktext) {
        ...
        file_prepare_draft_area($draftitemid, $context->id, FASTASSIGNFEEDBACK_COMMENTS_COMPONENT,
            FASTASSIGNFEEDBACK_COMMENTS_FILEAREA, $grade->id);
        ...
        $feedbackdata->fastassignfeedbackcomments_editor = [
            'text' => $feedbacktext,
            'format' => FORMAT_HTML,
            'itemid' => $draftitemid
        ];
    ...
    public function test_get_metadata() {
        $collection = new \core_privacy\local\metadata\collection('fastassignfeedback_comments');
        $collection = \fastassignfeedback_comments\privacy\provider::get_metadata($collection);
    ...
    public function test_export_feedback_user_data() {
        ...
        \fastassignfeedback_comments\privacy\provider::export_feedback_user_data($exportdata);
        ...
        $exportdata = new \mod_fastassignment\privacy\fastassignment_plugin_request_data($context, $fastassignment, $grade, [], $user2);
        \fastassignfeedback_comments\privacy\provider::export_feedback_user_data($exportdata);
        $this->assertContains($feedbacktext, $writer->get_data(['Feedback comments'])->commenttext);
    ...
    public function test_delete_feedback_for_context() {
        ...
        $fs = new file_storage();
        $files = $fs->get_area_files($fastassignment->get_context()->id, FASTASSIGNFEEDBACK_COMMENTS_COMPONENT,
        ...
        fastassignfeedback_comments\privacy\provider::delete_feedback_for_context($requestdata);
    ...
    public function test_delete_feedback_for_grade() {
        ...
        $requestdata = new \mod_fastassignment\privacy\fastassignment_plugin_request_data($context, $fastassignment, $grade1, [], $user1);
        fastassignfeedback_comments\privacy\provider::delete_feedback_for_grade($requestdata);
    ...
    public function test_delete_feedback_for_grades() {
        ...
        $fs = new file_storage();
        // 6 including directories for fastassignment 1.
        // 4 including directories for fastassignment 2.
        $this->assertCount(6, $fs->get_area_files($assign1->get_context()->id,
                FASTASSIGNFEEDBACK_COMMENTS_COMPONENT, FASTASSIGNFEEDBACK_COMMENTS_FILEAREA));
        $this->assertCount(4, $fs->get_area_files($assign2->get_context()->id,
                FASTASSIGNFEEDBACK_COMMENTS_COMPONENT, FASTASSIGNFEEDBACK_COMMENTS_FILEAREA));

        $deletedata = new \mod_fastassignment\privacy\fastassignment_plugin_request_data($assign1->get_context(), $assign1);
        $deletedata->set_userids([$user1->id, $user3->id]);
        $deletedata->populate_submissions_and_grades();
        fastassignfeedback_comments\privacy\provider::delete_feedback_for_grades($deletedata);
        ...
        // We have deleted two from fastassignment 1, and none from fastassignment 2.
        // 2 including directories for fastassignment 1.
        // 4 including directories for fastassignment 2.
