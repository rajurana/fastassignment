...
/**
 * This page handles listing of fastassignment overrides
 *
...
require_once($CFG->dirroot.'/mod/fastassignment/override_form.php');
...
$redirect = $CFG->wwwroot.'/mod/fastassignment/overrides.php?cmid=' . $cmid . '&amp;mode=group';

list($course, $cm) = get_course_and_cm_from_cmid($cmid, 'fastassignment');
$fastassignment = $DB->get_record('fastassignment', array('id' => $cm->instance), '*', MUST_EXIST);
...
require_capability('mod/fastassignment:manageoverrides', $context);
...
$overridecountgroup = $DB->count_records('fastassignment_overrides', array('userid' => null, 'assignid' => $fastassignment->id));
...
$url = new moodle_url('/mod/fastassignment/overrides.php', array('cmid' => $cm->id, 'mode' => $mode));
...
if ($action == 'movegroupoverride') {
    ...
    if (confirm_sesskey()) {
        move_group_override_fast($id, $dir, $fastassignment->id);
    }
    redirect($redirect);
}
...
$PAGE->set_title(get_string('overrides', 'fastassignment'));
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();
echo $OUTPUT->heading(format_string($fastassignment->name, true, array('context' => $context)));
...
$sql = 'SELECT o.id
     ...
     LEFT JOIN {groups} g ON o.groupid = g.id
         WHERE o.groupid IS NOT NULL
               AND g.id IS NULL
               AND o.assignid = ?';
$params = array($fastassignment->id);
$orphaned = $DB->get_records_sql($sql, $params);
if (!empty($orphaned)) {
    $DB->delete_records_list('fastassignment_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    if ($groups) {
        $params = ['assignid' => $fastassignment->id];
...
$table->head = array(
        $colname,
        get_string('overrides', 'fastassignment'),
        get_string('action'),
);
...
$overridedeleteurl = new moodle_url('/mod/fastassignment/overridedelete.php');
$overrideediturl = new moodle_url('/mod/fastassignment/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (isset($override->allowsubmissionsfromdate)) {
        $fields[] = get_string('open', 'fastassignment');
    ...
    if (isset($override->duedate)) {
        $fields[] = get_string('duedate', 'fastassignment');
        $values[] = $override->duedate > 0 ? userdate($override->duedate) : get_string('noclose', 'fastassignment');
    }

    // Format cutoffdate.
    if (isset($override->cutoffdate)) {
        $fields[] = get_string('cutoffdate', 'fastassignment');
        $values[] = $override->cutoffdate > 0 ? userdate($override->cutoffdate) : get_string('noclose', 'fastassignment');
    }
...
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'fastassignment'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'fastassignment'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'addgroup', 'cmid' => $cm->id)),
            get_string('addnewgroupoverride', 'fastassignment'), 'post', $options);
} else {
    $users = array();
    // See if there are any users in the fastassignment.
    if ($accessallgroups) {
        $users = get_enrolled_users($context, '', 0, 'u.id');
        $nousermessage = get_string('usersnone', 'fastassignment');
    } else if ($groups) {
        ...
        $nousermessage = get_string('groupsnone', 'fastassignment');
    }
    ...
    echo $OUTPUT->single_button($overrideediturl->out(true,
            ...
            get_string('addnewuseroverride', 'fastassignment'), 'get', $options);
}
