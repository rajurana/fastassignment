...
/**
 * This file contains the functions for fastassignment_plugin abstract class
 *
 *
...
/**
 * Abstract class for fastassignment_plugin (submission/feedback).
 *
 * @package   mod_fastassignment
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class fastassignment_plugin {

    /** @var fastassignment $assignment the assignment record that contains the global
     *              settings for this fastassignment instance
     */
     ...
     *
     * @param fastassignment $assignment
     * @param string $type
     */
    public final function __construct(fastassignment $assignment, $type) {
     ...
     *
     * @return string - either 'fastassignsubmission' or 'feedback'
     */
     ...
     *
     * @param stdClass $submissionorgrade - fastassignment_submission or fastassignment_grade.
    ...
    public final function has_admin_settings() {
        ...
        $pluginroot = $CFG->dirroot . '/mod/fastassignment/' . substr($this->get_subtype(), strlen('fastassign')) . '/' . $this->get_type();
    ...
    public final function set_config($name, $value) {
        ...
        $dbparams = array('assignment'=>$this->assignment->get_instance()->id,
                          ...
                          'name'=>$name);
        $current = $DB->get_record('fastassignment_plugin_config', $dbparams, '*', IGNORE_MISSING);

        if ($current) {
            $current->value = $value;
            return $DB->update_record('fastassignment_plugin_config', $current);
        } else {
            ...
            return $DB->insert_record('fastassignment_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            ...
            if ($assignment) {
                $dbparams = array('assignment'=>$assignment->id,
                                  ...
                                  'plugin'=>$this->get_type(),
                                  'name'=>$setting);
                $result = $DB->get_record('fastassignment_plugin_config', $dbparams, '*', IGNORE_MISSING);
                if ($result) {
                    return $result->value;
                }
            }
        ...
        $dbparams = array('assignment'=>$this->assignment->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                           'plugin'=>$this->get_type());
        $results = $DB->get_records('fastassignment_plugin_config', $dbparams);
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldassignment, & $log) {
        ...
        $log .= ' ' . get_string('upgradenotimplemented', 'mod_fastassignment', $params);
        return false;
    }
     ...
     * @param stdClass $submissionorgrade fastassignment_submission or fastassignment_grade The new submission or grade
     * @param string $log Record upgrade messages in the log
    ...
    public function upgrade(context $oldcontext,
                            ...
                            & $log) {
        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log = $log . ' ' . get_string('upgradenotimplemented', 'mod_fastassignment', $params);
        return false;
    }
    ...
    public function get_file_info($browser, $filearea, $itemid, $filepath, $filename) {
        ...
        if ($this->get_subtype() == 'fastassignsubmission') {
            if ($itemid) {
                $record = $DB->get_record('fastassignment_submission', array('id' => $itemid), 'userid,groupid', IGNORE_MISSING);
                if (!$record) {
                    return null;
                }
     ...
     *      id=coursemoduleid
     *      plugin=type
     *      pluginsubtype=fastassignfeedback|fastassignsubmission
