...
/**
 * Unit tests for (some of) mod/fastassignment/upgradelib.php.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_fastassignment_upgradelib_testcase extends advanced_testcase {
    ...
    public function test_upgrade_assignment($type, $plugins) {
        ...
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_fastassignmentment');
        ...
        $this->setAdminUser();
        $log = '';
        $upgrader = new fastassignment_upgrade_manager();

        $this->assertTrue($upgrader->upgrade_assignment($assignment->id, $log));
        $record = $DB->get_record('fastassignment', ['course' => $course->id]);

        $cm = get_coursemodule_from_instance('fastassignment', $record->id);
        $context = context_module::instance($cm->id);

        $fastassignment = new fastassignment($context, $cm, $course);

        foreach ($plugins as $plugin => $isempty) {
            $plugin = $fastassignment->get_submission_plugin_by_type($plugin);
