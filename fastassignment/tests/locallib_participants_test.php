...
class mod_fastassignment_locallib_participants extends advanced_testcase {
    public function test_list_participants_blind_marking() {
        ...
        fastassignment::allocate_unique_ids($fastassignment->get_instance()->id);

        $participants = $fastassignment->list_participants(null, false);
        ...
        $table = new fastassignment_grading_table($fastassignment, 10, '', 0, false);
        $table->setup();
        $table->query_db(10);
        ...
        static::helper_add_submission($fastassignment, $participants[$keys[1]], $data, 'onlinetext');

        // fastassignment has a private cache. The easiest way to clear this is to create a new instance.
        $fastassignment = new fastassignment($context, $cm, $course);

        $newparticipants = $fastassignment->list_participants(null, false);
        ...
        static::helper_add_submission($fastassignment, $participants[$keys[0]], $data, 'onlinetext');
        $fastassignment = new fastassignment($context, $cm, $course);
        $newparticipants = $fastassignment->list_participants(null, false);
    ...
    public function helper_add_submission($fastassignment, $user, $data, $type) {
        global $USER;

        $previoususer = $USER;

        $this->setUser($user);
        $submission = $fastassignment->get_user_submission($user->id, true);
        $submission->status = FASTASSIGN_SUBMISSION_STATUS_SUBMITTED;

        $rc = new ReflectionClass('fastassignment');
        $rcm = $rc->getMethod('update_submission');
        $rcm->setAccessible(true);
        $rcm->invokeArgs($fastassignment, [$submission, $user->id, true, false]);

        $plugin = $fastassignment->get_submission_plugin_by_type($type);
