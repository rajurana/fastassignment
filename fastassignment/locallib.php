...
$GLOBALS['vwtgrmrfeedback'] = null;

global $cm, $DB, $vwtgrmrfeedback;
...
class fastassignment {
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('fastassignment_user_mappng', array('assignment' => $this->get_instance()->id));
    ...
    private function get_submission_info_for_participants($participants) {
        ...
        if (!empty($this->get_instance()->blindmarking)) {
            $from .= 'LEFT JOIN {fastassignment_user_mappng} um
                             ON u.id = um.userid
    ...
    public function list_participants($currentgroup, $idsonly, $tablesort = false) {
        ...
        if (!isset($this->participants[$key])) {
            ...
            if (!empty($instance->blindmarking)) {
                $additionaljoins .= " LEFT JOIN {fastassignment_user_mappng} um
                                  ON u.id = um.userid
    ...
    public static function cron() {
        ...
        $sql = "SELECT g.id as gradeid, a.course, a.name, a.blindmarking, a.revealidentities, a.hidegrader,
            ...
            LEFT JOIN {fastassignment_user_mappng} um ON g.id = um.userid AND um.assignment = a.id
    ...
    protected function process_save_submission(&$mform, &$notices) {
        ...
        if ($data = $mform->get_data()) {
			...
			$_SESSION['api_messages'] = '';
			$activitty = $SESSION->activity;
			$role = 5;
			$get_api_teacher = $DB->get_record_sql("SELECT api_key, maxnoofchecks, maxnoofchecksstudent, maxnoofchecksteacher, test_name, test_links FROM {fastassignment} WHERE id = $activitty");
			
			$check_user_hits = $DB->get_record_sql("SELECT id, grammar_hits, autoeval_hits  FROM {fastassignment_api_validtn} WHERE activity = $activitty AND user = $userid AND role = $role");

            if(!!$data->grammarcheck) {
				
				$grammar_hits = ( ($check_user_hits->grammar_hits == 0) || empty($check_user_hits->grammar_hits) ) ? 1 : $check_user_hits->grammar_hits + 1;	
				$autoeval_hits = ( ($check_user_hits->autoeval_hits == 0) || empty($check_user_hits->grammar_hits) ) ? 0 : $check_user_hits->autoeval_hits;
				$this->fastassignment_api_hit($check_user_hits->id, $userid, $activity, $role, $grammar_hits, $autoeval_hits);
                
                $ch = curl_init();
                $curlConfig = array(
                    CURLOPT_URL            => "https://console.virtualwritingtutor.com/console/grammar/check-grammar",
                    CURLOPT_POST           => true,
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_POSTFIELDS     => json_encode([
                        'text' => strip_tags($data->onlinetext_editor['text'])
                    ])
                );
                $headers = [
                    'vwtapikey:'. $get_api_teacher->api_key,
                ...
                curl_close($ch);
				$decoded_resultt = json_decode($resultt);
				//echo '<pre>';print_r($decoded_resultt);echo '</pre>';die;
				$decodedresultt = $decoded_resultt->result->result;
				
				$messages = '';
				if(!empty($decodedresultt)){
					$messages .= "<div class='api-heading'> Grammar checker feedback: </div>";
					$i = 1;
					foreach($decodedresultt as $result){
						$suggest = '';
						foreach($result->suggestions as $suggestion){
							$suggest .= $suggestion.', ';
						}
						$suggest = rtrim($suggest, ', ');
						$messages .= "<ul class='list-inline language-accuracy-lists'>
							<li><b>$i. You wrote: </b>$result->context</li>
							<li><b>Feedback: </b>$result->feedback</li>
							<li><b>Error type: </b>$result->marked_text</li>
							<li><b>Suggestion: </b>$suggest</li>
						</ul>";
						$i++;
					}
				} else if(count($decodedresultt) === 0) {
				    $messages .= "<div class='api-heading'> Grammar checker feedback: </div><p>No errors detected.</p>";
				}
				$apimessages = isset($decoded_resultt->error) ? "<b>Error from API: </b><span class='api_error'>".$decoded_resultt->error."</span>" : $messages;
				
				$this->save_submission($data, $notices);
				
				$_SESSION['api_messages'] = $apimessages;
				$id = $data->id;
				$urltogo = new moodle_url("/mod/fastassignment/view.php?id=$id&action=editsubmission");
				redirect($urltogo);
            } 
            
            else if(!!$data->automatedevaluation) {
				
				$grammar_hits = ( ($check_user_hits->autoeval_hits == 0) || empty($check_user_hits->autoeval_hits) ) ? 0 : $check_user_hits->grammar_hits;
				$autoeval_hits = ( ($check_user_hits->autoeval_hits == 0) || empty($check_user_hits->autoeval_hits) ) ? 1 : $check_user_hits->autoeval_hits + 1;
				$this->fastassignment_api_hit($check_user_hits->id, $userid, $activity, $role, $grammar_hits, $autoeval_hits);
				
				$content = $data->onlinetext_editor['text'];
				...
				$essyTxt = preg_replace("/\r\n/", " ", $essyTxt);
				$essyTxt = str_replace("\xc2\xa0", "", $essyTxt);
                ...
                curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
                $curlConfig = array(
                    CURLOPT_URL            => "https://console.virtualwritingtutor.com/console/essay/test-feedback/v2",
                    CURLOPT_POST           => true,
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_POSTFIELDS     => json_encode([
                        'test_link' => $get_api_teacher->test_links,
                        'text' => $essyTxt
                    ])
                );
				/* echo "<pre>";
                ...
                $decoded_resultt = json_decode($resultt);
                /* echo "<pre>";
                print_r($decoded_resultt);
                echo "</pre>";die; */
				$decodedresultt = isset($decoded_resultt->result) ? $decoded_resultt->result : "<b>Error from API: </b><span class='api_error'>".$decoded_resultt->error."</span>";
				
				$this->save_submission($data, $notices);
				
				$_SESSION['api_messages'] = $decodedresultt;
				$id = $data->id;
				$urltogo = new moodle_url("/mod/fastassignment/view.php?id=$id&action=editsubmission");
				redirect($urltogo);
            } 
	...
	public function fastassignment_api_hit($id, $userid, $activity, $role, $grammar_hits, $autoeval_hits) {
		...
		if(!empty($id)){
			...
			//echo '<pre>';print_r($record);echo '</pre>';die('update_record');
			
			return $DB->update_record('fastassignment_api_validtn', $record);
		} else {
			
			//echo '<pre>';print_r($record);echo '</pre>';die('insert_record');
			
			return $DB->insert_record('fastassignment_api_validtn', $record);
		}
	}
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        global $USER, $CFG, $SESSION, $vwtgrmrfeedback;
    ...
    public static function allocate_unique_ids($assignid) {
        ...
        foreach ($users as $user) {
            $record = $DB->get_record('fastassignment_user_mappng',
            ...
            if (!$record) {
                ...
                $DB->insert_record('fastassignment_user_mappng', $record);
            }
        }
    }
    ...
    public static function get_uniqueid_for_user_static($assignid, $userid) {
        ...
        if ($record = $DB->get_record('fastassignment_user_mappng', $params, 'id')) {
            return $record->id;
        }
        ...
        return $DB->insert_record('fastassignment_user_mappng', $record);
    }
    ...
    public static function get_user_id_for_uniqueid_static($assignid, $uniqueid) {
        ...
        // Search for a record.
        if ($record = $DB->get_record('fastassignment_user_mappng',
